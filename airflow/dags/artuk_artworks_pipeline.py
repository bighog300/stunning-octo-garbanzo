...
SOURCE_DOMAIN = "artuk.org"
SPIDER_NAME = "artuk_artworks"
...
# airflow pools set artuk_pool 1 "Rate-limited Art UK crawl pool"
with DAG(
    ...
    dbt_run = BashOperator(
        task_id="dbt_run",
        bash_command="cd /opt/artio/dbt && dbt run --profiles-dir /opt/artio/dbt --select stg_artworks int_artworks_enriched mart_artworks stg_galleries int_gallery_normalized mart_galleries",
    )

    dbt_test = BashOperator(
        task_id="dbt_test",
        bash_command="cd /opt/artio/dbt && dbt test --profiles-dir /opt/artio/dbt --select stg_artworks mart_artworks stg_galleries mart_galleries",
    )
